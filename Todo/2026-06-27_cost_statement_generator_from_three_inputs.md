...
> 목적: sample1, sample2 원가계산보고서 형식을 참고해 `단가대비표`, `내역서`, `집계표` 3개 Excel 파일을 입력하면 수식이 연결된 `원가계산서` Excel workbook을 생성한다.
...
- [ ] GitHub Pages/Cloudflare Pages 배포 후 실제 URL 표시 확인

## 1. [패턴 확인] sample1/sample2 생성 기준 비교

**상세 내용:** sample1과 sample2 workbook은 같은 시트명과 수식 체인을 공유하며, `단가대비표`, `내역서`, `집계표`가 `원가계산서`, `결과`로 이어진다. 생성기는 우선 템플릿 workbook의 `원가계산서` 레이아웃과 수식을 유지하고, 3개 입력 시트의 XML cell data를 교체하는 방식으로 구현한다.

- [x] sample_ver1 최종 총액 확인: `원가계산서!E34 = 123,387,460`
- [x] sample_ver2 최종 총액 확인: `원가계산서!E34 = 109,104,460`
- [x] 공통 입력 시트 확정: `단가대비표`, `내역서`, `집계표`
...
- [x] CLI 인자 추가: `--price-comparison`, `--detail`, `--summary`, `--output`
- [x] 템플릿 선택 추가: `--template` 또는 `--template-version ver1|ver2`
- [x] 입력 workbook에서 정확한 시트명 우선 탐색
- [x] 시트명이 다를 때 기존 role 추론 로직으로 fallback
- [x] 템플릿의 `단가대비표`, `내역서`, `집계표` sheet XML cell data 교체
- [x] 출력 workbook 생성

## 3. [검증] sample 기반 smoke test

**상세 내용:** 현재는 독립된 3개 파일 대신 sample workbook 한 개를 세 입력 역할에 각각 넣어 round-trip 검증했다. 실제 운영 파일 3개가 준비되면 같은 CLI로 한 번 더 검증한다.
...
- [ ] 실제 분리된 `단가대비표.xlsx`, `내역서.xlsx`, `집계표.xlsx` 입력 파일로 end-to-end 검증
...
- [ ] GitHub Issue #41에 실제 사용자 파일 검증 결과 추가
