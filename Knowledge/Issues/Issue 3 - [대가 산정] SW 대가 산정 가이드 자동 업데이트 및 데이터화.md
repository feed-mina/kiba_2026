---
repo: feed-mina/kiba_2026
issue: 3
status: reference
github: https://github.com/feed-mina/kiba_2026/issues/3
tags:
  - issue
  - issue-3
  - project-status
---

# Issue 3 - [대가 산정] SW 대가 산정 가이드 자동 업데이트 및 데이터화

## 현재 상태

- 상태: `reference`
- 체크리스트: `0/0` 완료
- GitHub: [feed-mina/kiba_2026 Issue #3](https://github.com/feed-mina/kiba_2026/issues/3)
- 현황판: [index.html](file:///C:/Users/User/Desktop/KIBA/index.html)

## 다음 행동

- [ ] 다음 점검 시 Todo 또는 GitHub Issue에서 후속 행동을 확인

## 날짜 기록

- 연결된 Todo 파일 없음
## 관련 Todo

- 없음

## 관련 ASK

- [[ASK/2026-06-18_ai|2026-06-18 - 2026-06-18 Claude/Codex 질문·응답 로그]]

## 관련 docs / Cloud 링크

- 이슈 자료 목록 후보: [Cloud docs list](https://kiba.kibayerin.workers.dev/docs/list?repo=feed-mina%2Fkiba_2026&issue=3)

## 관련 index.html 카드

- [대가 산정] SW 대가 산정 가이드 자동 업데이트 및 데이터화 / source `kiba` / repo `feed-mina/kiba_2026`

#issue #project-status
